...
if [ "$auto" = "false" ]; then
    ...
    echo "Running installation in auto mode..."
fi
...
# If auto, bypass libgentl eula prompt
if [ "$auto" = "true" ]; then
    echo "Attempting to bypass libgentl eula prompt..."
    yes | sudo DEBIAN_FRONTEND=teletype dpkg -i libgentl_*.deb
else 
...
if [ "$auto" = "true" ]; then
    echo "Auto Installation complete. Run manually to access more installation options."
    exit 0
fi
